import { ContactForm } from '@/components/forms/ContactForm';
import { FadeIn } from '@/components/animations/FadeIn';
import { AnimatedSection } from '@/components/animations/AnimatedSection';
import type { Metadata } from 'next';

export const metadata: Metadata = {
  title: 'Contact Us | Pratik Gupta Interiors',
  description: 'Get in touch with Pratik Gupta Interiors to discuss your interior design project. We are here to bring your vision to life.',
};

/**
 * Contact Page
 * Contact form and studio information
 */

export default function ContactPage() {
  return (
    <div className="pt-32 pb-24 bg-background min-h-screen">
      <div className="container mx-auto px-6">
        {/* Page Header */}
        <AnimatedSection className="text-center mb-16">
          <p className="text-gold font-semibold uppercase tracking-wider text-sm mb-2">
            Get in Touch
          </p>
          <h1 className="text-5xl md:text-6xl font-serif font-bold text-charcoal mb-6">
            Contact Us
          </h1>
          <p className="text-xl text-charcoal/70 max-w-2xl mx-auto leading-relaxed">
            Let's discuss your project and explore how we can create a space that truly reflects your vision
          </p>
        </AnimatedSection>

        <div className="grid grid-cols-1 lg:grid-cols-5 gap-16">
          {/* Contact Information */}
          <FadeIn direction="left" className="lg:col-span-2">
            <div className="space-y-12">
              {/* Office Address */}
              <div>
                <h2 className="text-2xl font-serif font-bold text-charcoal mb-4">
                  Our Studio
                </h2>
                <div className="space-y-2 text-charcoal/70">
                  <p>Pratik Gupta Interiors</p>
                  <p>123 Design District</p>
                  <p>Lower Parel, Mumbai</p>
                  <p>Maharashtra 400013, India</p>
                </div>
              </div>

              {/* Contact Details */}
              <div>
                <h2 className="text-2xl font-serif font-bold text-charcoal mb-4">
                  Contact Details
                </h2>
                <div className="space-y-4">
                  <div>
                    <p className="text-sm text-charcoal/60 font-semibold uppercase tracking-wide mb-1">
                      Email
                    </p>
                    <a
                      href="mailto:hello@pratikguptainteriors.com"
                      className="text-charcoal hover:text-gold transition-colors"
                    >
                      hello@pratikguptainteriors.com
                    </a>
                  </div>
                  <div>
                    <p className="text-sm text-charcoal/60 font-semibold uppercase tracking-wide mb-1">
                      Phone
                    </p>
                    <a
                      href="tel:+919876543210"
                      className="text-charcoal hover:text-gold transition-colors"
                    >
                      +91 98765 43210
                    </a>
                  </div>
                  <div>
                    <p className="text-sm text-charcoal/60 font-semibold uppercase tracking-wide mb-1">
                      WhatsApp
                    </p>
                    <a
                      href="https://wa.me/919876543210"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-charcoal hover:text-gold transition-colors"
                    >
                      +91 98765 43210
                    </a>
                  </div>
                </div>
              </div>

              {/* Office Hours */}
              <div>
                <h2 className="text-2xl font-serif font-bold text-charcoal mb-4">
                  Office Hours
                </h2>
                <div className="space-y-2 text-charcoal/70">
                  <div className="flex justify-between">
                    <span>Monday - Friday</span>
                    <span>10:00 AM - 6:00 PM</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Saturday</span>
                    <span>11:00 AM - 4:00 PM</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Sunday</span>
                    <span>Closed</span>
                  </div>
                </div>
              </div>

              {/* Social Media */}
              <div>
                <h2 className="text-2xl font-serif font-bold text-charcoal mb-4">
                  Follow Us
                </h2>
                <div className="flex items-center gap-4">
                  <a
                    href="https://instagram.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 rounded-full border-2 border-charcoal flex items-center justify-center hover:bg-gold hover:border-gold transition-all duration-300 group"
                    aria-label="Instagram"
                  >
                    <svg
                      className="w-5 h-5 text-charcoal group-hover:text-charcoal"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                    </svg>
                  </a>
                  <a
                    href="https://facebook.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 rounded-full border-2 border-charcoal flex items-center justify-center hover:bg-gold hover:border-gold transition-all duration-300 group"
                    aria-label="Facebook"
                  >
                    <svg
                      className="w-5 h-5 text-charcoal group-hover:text-charcoal"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                    </svg>
                  </a>
                  <a
                    href="https://pinterest.com"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-10 h-10 rounded-full border-2 border-charcoal flex items-center justify-center hover:bg-gold hover:border-gold transition-all duration-300 group"
                    aria-label="Pinterest"
                  >
                    <svg
                      className="w-5 h-5 text-charcoal group-hover:text-charcoal"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 0c-6.627 0-12 5.372-12 12 0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146 1.124.347 2.317.535 3.554.535 6.627 0 12-5.373 12-12 0-6.628-5.373-12-12-12z" />
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </FadeIn>

          {/* Contact Form */}
          <FadeIn direction="right" className="lg:col-span-3">
            <div className="bg-off-white p-8 md:p-12 border border-charcoal/10">
              <h2 className="text-3xl font-serif font-bold text-charcoal mb-2">
                Start Your Project
              </h2>
              <p className="text-charcoal/70 mb-8">
                Fill out the form below and we'll get back to you within 24 hours
              </p>
              <ContactForm />
            </div>
          </FadeIn>
        </div>

        {/* Map Section (Placeholder) */}
        <FadeIn className="mt-24">
          <div className="bg-charcoal/5 h-96 flex items-center justify-center">
            <div className="text-center text-charcoal/60">
              <svg
                className="w-16 h-16 mx-auto mb-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
                />
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
                />
              </svg>
              <p className="text-lg">Google Maps Integration</p>
              <p className="text-sm">(Replace with actual map embed)</p>
            </div>
          </div>
        </FadeIn>
      </div>
    </div>
  );
}
